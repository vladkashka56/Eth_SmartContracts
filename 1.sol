...
contract MAMATOKEN is Context, IERC20, Ownable {
    ...
    uint256 private _previousMkFee = _marketingFee;
    uint256 public _devFee = 4;
    uint256 private _previousDevFee = _devFee;
    uint256 public _liquidityFee = 3;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public _maxWalletAmount = 12 * 10**5;
    ...
    function setMaxWalletAmount(uint256 maxWalletAmount, uint256 maxWalletDecimal) external onlyOwner {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, fees.tFee, fees.tDev, fees.tLiquidity, _getRate());
        return (rAmount, rTransferAmount, rFee, fees.tTransferAmount, fees.tFee, fees.tDev, fees.tLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (Fees memory) {
        ...
        fees.tTransferAmount = tAmount.sub(fees.tFee).sub(fees.tDev).sub(fees.tLiquidity);
        return (fees);
    }
    ...
    function _takeCharity(uint256 tCharity) private lockTheSwap {
        uint256 currentRate =  _getRate();
        ...
        if (tCharity > 0) {
            ...
            // swap tokens for ETH
            swapTokensForEth(tCharity); // <- this breaks the ETH -> HATE swap when swap + liquify is triggered

            // how much ETH did we just swap into?
            uint256 newBalance = address(this).balance.sub(initialBalance);
            
            TransferBnbToExternalAddress(_charityWalletAddress, newBalance);
        }
    }
    
    function _takeDev(uint256 tDev) private lockTheSwap {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        ...
        if (tDev > 0) {
            ...
            // swap tokens for ETH
            swapTokensForEth(tDev); // <- this breaks the ETH -> HATE swap when swap + liquify is triggered

            // how much ETH did we just swap into?
            uint256 newBalance = address(this).balance.sub(initialBalance);
            
            TransferBnbToExternalAddress(_devWalletAddress, newBalance);
        }
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != address(1) && to != uniswapV2Pair && to != _devWalletAddress) {      
